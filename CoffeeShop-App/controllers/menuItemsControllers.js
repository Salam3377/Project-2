...
// 		// if they're logged in, go to the next thing(thats the controller) comment for now
// 		next() 
// 	} else {
...
router.get('/', (req, res) => {
	MenuItems.find({})
		.then(items => {
			...
			res.render('menu/index', { items, username, loggedIn })
		})
		.catch(error => {
...
router.get('/new', (req, res) => {
	const { username, userId, loggedIn } = req.session
	res.render('menu/new', { username, loggedIn,userId })
})
...
router.get('/:id', (req, res) => {
	...
	MenuItems.findById(menuItemsId)
		.then(items => {
            const {username, loggedIn, userId} = req.session
			res.render('menu/show', { example, username, loggedIn, userId })
		})
		.catch((error) => {
			res.redirect(`/error?error=${error}`)
		})
